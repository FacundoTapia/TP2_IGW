...
public interface I_ClienteRepository {
    ...
    default List<Cliente> getLikeNombreApellidoDireccionTipoDocumento(String nombre, String apellido, String direccion, String tipoDocumento){
        if(apellido == null || nombre == null || direccion == null || tipoDocumento == null ) return new ArrayList();
        return getAll()
                .stream()
                .filter(c->c!=null && c.getApellido() != null && c.getNombre() != null && c.getDireccion() != null && c.getTipoDocumento() != null &&
                        c.getApellido().toLowerCase().contains(apellido.toLowerCase()) &&
                        c.getNombre().toLowerCase().contains(nombre.toLowerCase()) &&
                        c.getDireccion().toLowerCase().contains(direccion.toLowerCase())&&
    ...
    default Cliente getByDni(int numeroDocumento){
        return getAll()
                .stream()
                .filter(c->c.getNumeroDocumento()==numeroDocumento)
                .findAny()
                .orElse(new Cliente());
    }
    default Cliente getByTipoNumeroDocumento(TipoDocumento tipoDocumento, int numeroDocumento){
        if(tipoDocumento == null) new ArrayList();
        return getAll()
                .stream()
                .filter(c->c!=null && c.getTipoDocumento() != null &&
                        c.getTipoDocumento().equals(tipoDocumento) &&
                        c.getNumeroDocumento()==numeroDocumento)
                .findAny()
                .orElse(new Cliente());
    }
}